...
def create_fake_venues(name, city, addr, zip, lat, long, url):
...
def create_fake_event(venue, desc, opener, headliner):
	...
	print '{venue: %s, description: %s, opener: %s, headliner: %s}' % (venue, description, opener, headliner)

	event_info = {'venue': venue, 'description': description, 'opener': opener, 'headliner': headliner }
	
	return json.dumps( event_info )

def fake_it( num_events=10000, num_venues=150, num_artists=500, hrp="/user/sceneFindr/history/"):
	fake = Factory.create()
	...
	for v in range(num_venues):
		...
		venue = create_fake_venues(name, city, addr, zip, lat, long, url)
		for event in range(num_events):
			#each venue has events
			local_filename = "venue_%s_event_%s.txt" % (v, event)
			#open filewriter
